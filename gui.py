...
class GUI(LayeredUpdates):
    ...
    def gather_pressed(self):
        ...
        if self.sel_unit.carrying:
            ...
            gathered = gather.gather(self.sel_unit,
                                     eligible,
                                     lambda u : (u.health))
    ...
    def __init__(self, screen_rect, bg_color):
        ...
        self.buttons = [
            ...
            Button(3, "GATHER/UNLOAD", self.gather_pressed, None)]
    ...
    def load_level(self, filename):
        ...
        while line.find("UNITS END") < 0:
            ...
            if len(line) != 5 and len(line) != 6:
                raise Exception("Invalid definition of " + unit_name)
            
            if len(line) == 6 and unit_name != "Transport":
                raise Exception("Invalid definition of " + unit_name)
        ...
        if (e.type == pygame.MOUSEBUTTONUP
            ...
            if self.map.rect.collidepoint(e.pos):
                ...
                if unit:
                    ...
                    # if we are moving and this tile is movable
                    # then it must be a transport on our team with
                    # enough capacity
                    elif (self.mode == Modes.ChooseMove and
                          to_tile_pos in self._movable_tiles):
                        self.sel_unit_move(to_tile_pos)
                        for u in base_unit.BaseUnit.active_units:
                            if (u.tile_x, u.tile_y) == to_tile_pos:
                                self.sel_unit.in_transport = u
                        self.sel_unit = None
    ...
    def draw_bar(self):
        ...
        if self.sel_unit:
            ...
            if self.sel_unit.type != "Transport" and \
              isinstance(self.sel_unit, unit.ground_unit.GroundUnit):
                size = self.sel_unit.unit_size
                self.draw_bar_text(
                    "Size: {}".format(size), line_num)
                line_num += 1

            if self.sel_unit.type == "Transport":
                carrying = len(self.sel_unit.carrying)
                capacity = self.sel_unit.capacity
                self.draw_bar_text(
                    "Units: {}  |  Space: {}".format(carrying, capacity),
                    line_num)
                line_num += 1

                if self.sel_unit.carrying:
                    front_u = self.sel_unit.carrying[0]
                    self.draw_bar_text(
                        "Front: {} | Health: {}".format(front_u.type, 
                                                        front_u.health),
                        line_num)
                    line_num += 1
        ...
        if hov_unit:
            ...
            if hov_unit.type != "Transport" and \
              isinstance(hov_unit, unit.ground_unit.GroundUnit):
                size = hov_unit.unit_size
                self.draw_bar_text(
                    "Size: {}".format(size), line_num)
                line_num += 1

            if hov_unit.type == "Transport":
                carrying = len(hov_unit.carrying)
                capacity = hov_unit.capacity
                self.draw_bar_text(
                    "Units: {}  |  Space: {}".format(carrying, capacity),
                    line_num)
                line_num += 1

                if hov_unit.carrying:
                    front_u = hov_unit.carrying[0]
                    self.draw_bar_text(
                        "Front: {} | Health: {}".format(front_u.type, 
                                                        front_u.health),
                        line_num)
        ...
        for button in self.buttons:
            if self.sel_unit and self.sel_unit.type == "Transport":
